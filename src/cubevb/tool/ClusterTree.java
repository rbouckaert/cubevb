...
public class ClusterTree extends Tree implements StateNodeInitialiser {
    ...
    protected Node newNode() {
    	// reuse nodes if already created earlier
    	if (m_nodes != null && m_nodes[lastNode+1] != null) {    		
    		lastNode++;
    		Node node = m_nodes[lastNode];
    ...
    class Tuple {
        ...
        public Tuple(final double d, final int i, final int j, final int row, final int col, final int size1, final int size2) {
            m_fDist = d;
            m_iCluster1 = i;
            m_iCluster2 = j;
            m_nClusterSize1 = size1;
        ...
        @Override
		public String toString() {
			return row +"x" + col + " (" + m_iCluster1 +"[" + m_nClusterSize1 +"]x" + m_iCluster2 + "[" + m_nClusterSize2 +"]) " + m_fDist + "\n";
		}
    }
    ...
    void doLinkClustering(int clusters, final List<Integer>[] clusterID, final NodeX[] clusterNodes) {
		...
		int [] colCount = new int[clusters];

		int [] node2cluster = new int[clusters];
        for (int i = 0; i < clusters; i++) {
        	node2cluster[i] = i;
        }
        ...
        while (clusters > 1) {
            ...
            Tuple t = queue.poll();
            min1 = node2cluster[t.m_iCluster1];
            min2 = node2cluster[t.m_iCluster2];
            if (min1 != min2 
//            		&& colCount[t.row] < 2 && colCount[t.col] < 2
            		) {
            	merge(min1, min2, t.m_fDist/2.0, t.m_fDist/2.0, clusterID, clusterNodes);
            	//System.err.print(t.row + "x" + t.col +"\t");
    			colCount[t.row]++;
    			colCount[t.col]++; 
                ...
                for (int d : clusterID[min1]) {
        			node2cluster[d] = min1;
        		}
                for (int d : clusterID[min2]) {
        			node2cluster[d] = min2;
        		}
            }
        }
